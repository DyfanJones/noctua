...
Athena_write_table <-
  function(conn, name, value, overwrite = FALSE, append = FALSE,
    ...
    if (found && append) {
      ...
      File.Type <- switch(tbl_info$Parameters$serde.serialization.lib,
        "org.apache.hadoop.hive.serde2.lazy.LazySimpleSerDe" = {
          switch(tbl_info$Parameters$serde.param.field.delim,
            "," = "csv",
            "\t" = "tsv",
      ...
      if (file.type != File.Type) {
        warning(
          ...
          ' file type and has been converted to "', File.Type, '".',
          call. = FALSE
        )
      }
    ...
    if (!append) {
      sql <- sqlCreateTable(conn,
        table = db_name, fields = value, field.types = field.types,
